...
def find_branch_pts(skel_img):
    ...
    :param label: str
    :return branch_dict-> [index,[[point],[neighbors]]]
    """
    ...
    branch_pts_img = np.zeros(skel_img_padded.shape[:2], dtype=int)


    # Store branch points
    for kernel in kernels:
        branch_pts_img = np.logical_or(cv2.morphologyEx(skel_img_padded, op=cv2.MORPH_HITMISS, kernel=kernel,
